...
def importVaultOfSatoshi(bid_currency, quote_currency, book_data, content, file_time):
    if (content['status'] != "success"):
        raise ExchangeError('Unsuccessful response from Vault of Satoshi')
    
    asks = []
    bids = []
    market_data = content['data']
    
    # Note we have to check precision as some captured
    # data is at too low a precision to be useful
    if (len(market_data['bids']) == 1 and market_data['bids'][0]['price']['value'] == '0.00'):
        return
    
    for order in market_data['asks']:
        asks.append([order['price']['value'], order['quantity']['value']])
    
    for order in market_data['bids']:
        bids.append([order['price']['value'], order['quantity']['value']])
...
base_dir = "/home/jrn/cryptocurrency_data/doge_book_data"
...
for hour in [ d for d in listdir(base_dir) if isdir(join(base_dir, d)) ]:
    ...
    for exchange in [ d for d in listdir(hour_dir) if isdir(join(hour_dir, d)) ]:
        ...
        for data_file in [ f for f in listdir(exchange_dir) if isfile(join(exchange_dir, f)) ]:
            ...
            existing_file = imported_files.find_one({"market": "DOGE/BTC",
                "exchange": exchange,
                "filename": data_file})
            ...
            try:
                ...
                if (exchange == "Cryptsy"):
                    importCryptsy("DOGE", "BTC", book_data, content, file_time)
                elif (exchange == "Vircurex"):
                    importVircurex("DOGE", "BTC", book_data, content, file_time)
                elif (exchange == "VoS"):
                    importVaultOfSatoshi("DOGE", "USD", book_data, content, file_time)
